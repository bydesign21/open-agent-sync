...
pub struct Generated {
    pub ops: Vec<FsOp>,
    /// Exact local marketplace root the operations target.
    pub marketplace_dir: PathBuf,
...
impl Generated {
    /// Check that every generated artifact still matches the bytes or link
    /// target the generator promises for the observable source handlers.
    ///
    /// This deliberately consumes the same [`FsOp`] contract as apply. A
    /// parallel parser for hooks.json or sidecars could agree with a stale
    /// artifact while the generator would replace it, recreating the
    /// self-confirming loop this guard exists to break.
    pub fn verify_on_disk(&self) -> Result<()> {
        for op in &self.ops {
            match op {
                FsOp::WriteFile { path, contents } => {
                    let actual = std::fs::read_to_string(path).with_context(|| {
                        format!("reading generated artifact {}", path.display())
                    })?;
                    if actual != *contents {
                        bail!(
                            "generated artifact {} does not match the current generation contract",
                            path.display()
                        );
                    }
                }
                FsOp::Link { target, link } => {
                    let actual = std::fs::read_link(link)
                        .with_context(|| format!("reading generated link {}", link.display()))?;
                    if actual != *target {
                        bail!(
                            "generated link {} points to {} instead of {}",
                            link.display(),
                            actual.display(),
                            target.display()
                        );
                ...
                other => bail!("generation contract contains an unsupported operation: {other:?}"),
            }
        }
        verify_marketplace_entry(&self.marketplace_dir, &self.shim_plugin)
    }
}
...
pub fn plan_shim(input: &ShimInput) -> Result<Generated> {
    ...
    Ok(Generated {
        ops,
        marketplace_dir: input.marketplace_dir.clone(),
        shim_plugin,
...
pub fn marketplace_manifest_op(marketplace_dir: &Path, shim_plugins: &[String]) -> Result<FsOp> {
    let marketplace = json!({
        ...
        "plugins": shim_plugins
            .iter()
            .map(|name| marketplace_entry(name))
            .collect::<Vec<_>>(),
    });
    Ok(FsOp::WriteFile {
...
fn marketplace_entry(name: &str) -> Value {
    json!({
        "name": name,
        // The generator writes each shim to
        // `{marketplace_dir}/{shim_plugin_name}/`, so the source is that
        // directory, relative to the marketplace root. A source that points
        // anywhere else is the same bug in a new costume.
        "source": format!("./{name}"),
        "description": format!(
...
fn verify_marketplace_entry(marketplace_dir: &Path, shim_plugin: &str) -> Result<()> {
    let path = marketplace_dir.join(".claude-plugin/marketplace.json");
    let text = std::fs::read_to_string(&path)
        .with_context(|| format!("reading generated marketplace {}", path.display()))?;
    let manifest: Value = serde_json::from_str(&text)
        .with_context(|| format!("parsing generated marketplace {}", path.display()))?;
    if manifest.get("name").and_then(Value::as_str) != Some(MARKETPLACE_NAME) {
        bail!(
            "generated marketplace {} has the wrong name",
            path.display()
        );
    }
    let expected = marketplace_entry(shim_plugin);
    if !manifest
        .get("plugins")
        .and_then(Value::as_array)
        .is_some_and(|plugins| plugins.iter().any(|entry| entry == &expected))
    {
        bail!(
            "generated marketplace {} does not contain the expected entry for {shim_plugin}",
            path.display()
        );
    }
    Ok(())
