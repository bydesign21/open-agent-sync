...
// ---------------------------------------------------------------------------
//
// Unlike every other host in this file, OpenCode has no marketplace and no
// CLI plugin/install command at all (measured; see `docs/open-work.md`), so
// there is no fake host script here: the whole write path is one guarded
// `FileTransaction` (`src/shim/bridges/opencode.rs`), never a `Step::Host`.
